...
class BLEPeripheral:
    ...
    UUID_SEND = "851A4582-19C1-4E6C-AB37-E7A03766BA16"
    ...
    def receive_ikawaapp_command(self, value, characteristic):
        ...
        if value[0] != 126 and len(self.long_message) == 0:
        ...
        crc_value = message[-2:]
        message = message[:-2]
        message_crc = self.crc16(message, 65535)

        if message_crc != crc_value:
            print("Error: CRC Error")
            message_crc = self.crc16(message, 43690)
            if message_crc == crc_value:
        ...
        if respType is not None:
            try:
                self.send_message_queue.put(respType.SerializeToString())
            except Exception as e:
        ...
        if not self.send_message_queue.empty():
            try:
                ...
                for i in range(0, len(message), message_size_limit):
                    ...
                    characteristic.set_value(bytes_array)
            except Exception as e:
    ...
    def __init__(self, adapter_address, roaster):
        ...
        self.RoasterPeripheral = peripheral.Peripheral(
            adapter_address, local_name=self.advertised_name, appearance=0x0340
        )
        self.RoasterPeripheral.add_service(srv_id=1, uuid=self.IKAWA_SERVICE, primary=True)

        self.RoasterPeripheral.add_characteristic(
            srv_id=1,
            chr_id=1,
            uuid=self.UUID_RECEIVE,
            value=[],
            notifying=False,
            flags=["notify"],
            read_callback=None,
        ...
        self.RoasterPeripheral.add_characteristic(
            srv_id=1,
            chr_id=3,
            uuid=self.UUID_CLIENT_CONFIGURATION,
            value=[],
            notifying=False,
